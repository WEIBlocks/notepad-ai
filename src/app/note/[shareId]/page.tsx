...
export default function ViewNote() {
  ...
  const [note, setNote] = useState<{ content: string; title: string; allowEditing: boolean } | null>(null);
  ...
  if (requiresPassword) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center min-h-screen p-4 bg-[#1a1f2e]/80 backdrop-blur-md">
        <div className="bg-[#1a1f2e] rounded-lg p-6 sm:p-8 max-w-md w-full shadow-2xl border border-[#2a3142]/50 animate-fade-in-up">
          <h2 className="text-xl sm:text-2xl text-center font-bold text-white mb-6 bg-gradient-to-r from-blue-400/80 to-purple-600/80 bg-clip-text text-transparent relative">
            ...
            <span className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-blue-400/60 to-purple-600/60 animate-pulse-width" />
          </h2>
          <form onSubmit={handlePasswordSubmit} className="relative space-y-8 z-10">
            {/* Gradient Overlay with Pulse */}
            <div className="absolute inset-0 rounded-lg bg-gradient-to-br from-[#1a1f2e]/70 to-[#151823]/70 opacity-50 blur-sm z-0 animate-pulse" />
            ...
            <button
              type="submit"
              className="w-full bg-gradient-to-r from-blue-700/90 to-purple-700/90 text-white rounded-xl py-3 font-semibold relative overflow-hidden group shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-transform duration-200"
            >
              <span className="relative z-10">Access Note</span>
              <div className="absolute inset-0 bg-gradient-to-r from-blue-500/40 to-purple-500/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
            </button>
          </form>
        </div>
